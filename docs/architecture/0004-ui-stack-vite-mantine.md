...
## Context
ADR-0003 decided the UI is a **React Router** application **served by the Micronaut
...
- **Single artifact, single origin (ADR-0003).** The backend serves the UI's built
...
- **JVM + Node toolchains (ADR-0003).** The build pipeline already spans two
...
- **Routing: React Router in library mode** — `createBrowserRouter` + `RouterProvider`,
  ...
  "static assets served by Micronaut" delivery in ADR-0003.
...
- **Location: `ui/`** (per the README's module layout), a sibling of `server/`.
...
## Consequences
+ Static-asset output drops straight into the single Micronaut artifact (ADR-0003)
