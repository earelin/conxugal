...
`ADMIN` endpoints of [TASK-0006](TASK-0006-taxonomia-admin-endpoints.md). Governed by
...
Visual target: [`design/create-termo.svg`](design/create-termo.svg),
[`design/move-termo.svg`](design/move-termo.svg),
[`design/delete-termo.svg`](design/delete-termo.svg), plus the selected-term action row in
[`design/taxonomia-admin.svg`](design/taxonomia-admin.svg).

## Scope
- `Novo termo` in the tree header, and inline **rename / move / delete** actions on the
...
- **Selecting an invalid target explains, it does not hide.** Picking the term itself or one
  of its descendants is *selectable*, and doing so renders the cycle alert naming both terms
  with the primary action disabled — the behaviour `design/move-termo.svg` shows. Filtering
...
- After a mutation, refetch the taxonomy read and re-run TASK-0007's builder; there is no
  ...
  returns Órganos to unclassified. A create selects and expands the new term using the id
...
- **Surface each refusal as its own message**, keyed on the problem `type` from TASK-0006,
  ...
  - `termo-cycle` → a term cannot be moved under itself or its own descendant.
  - `termo-has-children` → move or delete the child terms first.
  - `duplicate-sibling-name` → a sibling already has that name.
  - `termo-not-found` → the term was deleted by someone else; refresh.
...
## Acceptance criteria
- An `ADMIN` can create a term at the root and under a parent, rename it, move it to
...
- A move that would create a cycle and a delete of a term with children each render their
...
- Selecting the term itself, or a descendant, as a move target is possible and produces the
...
- Acting on a term another admin has just deleted shows the stale-reference message and
...
- Deleting a term that holds Órganos returns them to the unclassified worklist in the same
