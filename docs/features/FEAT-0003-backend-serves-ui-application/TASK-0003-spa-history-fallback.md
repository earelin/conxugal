...
(backend-owned SPA fallback). Closes the production gap the UI scaffolding flagged
as a dependency on this feature.
...
- Applies uniformly to both known SPA routes (e.g. `/acerca`) and unknown ones — React
  Router's own `NotFoundPage` renders the in-shell Galician not-found state
